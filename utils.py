...
def calc_delta_c(params, particle_radii, particle_mass, critical_density, Rs, R_200):
    ...
    while res > tol:
        Rvir += dr
        Mvir = particle_mass * len(np.argwhere((particle_radii < Rvir)).flatten())
...
def vel_disp_nfw(x, conc, beta):
    integrand = lambda s, b, c : ((s**(2 * b - 3) * np.log(1 + c * s)) / (1 + c * s)**2) -\
        ((c * s**(2 * b - 2)) / (1 + c * s)**3)
    dispint = []
    for ri in x:
